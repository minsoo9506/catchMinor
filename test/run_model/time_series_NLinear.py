...
from catchMinor.time_series_model.NLinear.lit_NLinear import LitNLinear
...
def define_argparser():
    ...
    parser.add_argument("--model", default="NLinear")
    parser.add_argument(
        "--batch_size",
        type=int,
        default=1024,
...
if __name__ == "__main__":
    ...
    # config
    model_config = NLinear_config(seq_len=128, pred_len=128)
    loss_config = NLinear_loss_func_config(optimizer_params={"lr": 0.001})
    optim_config = NLinear_optimizer_config()

    model = LitNLinear(model_config, optim_config, loss_config)
    ...
    with torch.no_grad():
        ...
        for batch in test_loader:
            anomaly_score = (
                model.get_anomaly_score(batch, "time_series")
                .detach()
                .numpy()
                .flatten()
                .tolist()
            )
            # .flatten(): (batch_size, seq_len) -> (batch_size * seq_len)
            anomaly_scores += anomaly_score

    # not overlap time series case
