...
class WindowDataset(Dataset):
    ...
    def __init__(self, x: np.ndarray, y: np.ndarray, window_size: int, overlaps: bool):
        """
        Args:
        ...
        self.x = self._make_window_based_data(x, window_size, overlaps)
        self.y = self._make_window_based_data(y, window_size, overlaps)
    ...
    def _make_window_based_data(
        self, data: np.ndarray, window_size: int, overlaps: int
    ) -> np.ndarray:
        if data.ndim == 1:
            data = np.expand_dims(data, axis=-1)
        if overlaps:
            _data = self._overlaps(data, window_size)
        else:
            _data = self._non_overlaps(data, window_size)
        return _data

    def _overlaps(self, data: np.ndarray, window_size: int) -> np.ndarray:
    ...
    def _non_overlaps(self, data: np.ndarray, window_size: int) -> np.ndarray:
